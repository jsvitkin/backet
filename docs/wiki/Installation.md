...
For Discord bot hosting, the stronger `rag-standard` and `rag-quality` profiles can also use self-hosted embedding, reranker, and answer model services configured in the bot runtime profile. Those services are separate from the base CLI install and are covered in [[Hosting Backet Bot]].
