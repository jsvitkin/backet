...
def test_update_check_command_supports_json_and_human_output(runner, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_urlopen(*_args: object, **_kwargs: object) -> FakeResponse:
        calls.append("network")
        return FakeResponse(fake_latest_response("0.1.10"))
    ...
    assert payload["data"]["latest_version"] == "0.1.10"
...
def test_update_apply_yes_runs_pipx_install_with_resolved_wheel(
    ...
    monkeypatch.setattr(cli_update, "urlopen", lambda *_args, **_kwargs: FakeResponse(fake_latest_response("0.1.10")))
    ...
    assert commands == [
        [
            ...
            "install",
            "--force",
            "https://github.com/jsvitkin/backet/releases/download/v0.1.10/backet-0.1.10-py3-none-any.whl",
        ]
    ]
