...
def test_distribution_metadata_loads_from_repo() -> None:
    ...
    assert metadata.resolved_ref() == "v0.1.17"
    assert metadata.release_artifact_name("0.1.17") == "backet-0.1.17-py3-none-any.whl"


def test_distribution_metadata_builds_release_url() -> None:
    metadata = load_distribution_metadata()

    url = metadata.release_artifact_url("0.1.17", repository="example/backet")

    assert url == "https://github.com/example/backet/releases/download/v0.1.17/backet-0.1.17-py3-none-any.whl"
...
def test_skills_archive_url_resolves_release_tags_and_branch_refs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        ...
        == "https://codeload.github.com/example/backet/zip/refs/tags/v0.1.17"
    )
    assert (
